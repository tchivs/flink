...
import io.confluent.flink.table.modules.search.providers.SearchProviderSelector;
import io.confluent.flink.table.modules.search.providers.SearchRuntimeProvider;
...
public class VectorSearchRuntime implements AutoCloseable {
    ...
    VectorSearchRuntime(
        ...
        this.clock = clock;
        this.provider = pickProvider(table, configuration);
    ...
    private SearchRuntimeProvider pickProvider(
            CatalogTable table, Map<String, String> configuration) {
    ...
    public static VectorSearchRuntime open(
        ...
        // With the first retry, wait 2 seconds. Each subsequent retry will double the wait time.
        final long maxRetryWait = Duration.ofSeconds(2).toMillis();
        final OkHttpClient httpClient =
                new OkHttpClient.Builder()
                        ...
                        .build();
        return new VectorSearchRuntime(table, configuration, httpClient, metrics, clock);
    }

    @VisibleForTesting
    public static VectorSearchRuntime mockOpen(
            CatalogTable table,
            Map<String, String> configuration,
            OkHttpClient httpClient,
            MLFunctionMetrics metrics,
    ...
    public static VectorSearchRuntime mockOpen(
            ...
            Clock clock) {
        // We don't add the retry interceptor in tests, as that would break the http mock.
        return new VectorSearchRuntime(provider, httpClient, metrics, clock);
    }

    /** Interceptor to retry requests on quota errors. */
    public static class VectorSearchRetryInterceptor implements Interceptor {
        private int maxRetries;
        private long retryWait;

        public VectorSearchRetryInterceptor(int maxRetries, long retryWaitMilliseconds) {
    ...
    public Row run(Object[] args) throws Exception {
        ...
        try {
            request = provider.getRequest(args);
        } catch (Throwable t) {
