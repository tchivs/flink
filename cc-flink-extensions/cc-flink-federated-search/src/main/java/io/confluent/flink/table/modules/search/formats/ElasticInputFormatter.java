...
public class ElasticInputFormatter extends JsonObjectInputFormatter {
    ...
    static List<Schema.UnresolvedColumn> getElasticInputColumns(
        ...
        LogicalType fieldType = MlUtils.getLogicalType(inputColumns.get(0));
        if (fieldType.getTypeRoot() != LogicalTypeRoot.VARCHAR) {
            throw new FlinkRuntimeException(
                    "Elastic field input must be a string, got " + fieldType);
        }
        LogicalType kType = MlUtils.getLogicalType(inputColumns.get(1));
        ...
        LogicalType queryVectorType = MlUtils.getLogicalType(inputColumns.get(2));
        ...
        unresolvedColumns.add(
                new Schema.UnresolvedPhysicalColumn(
                        "field", MlUtils.getAbstractType(inputColumns.get(0))));
        unresolvedColumns.add(
                new Schema.UnresolvedPhysicalColumn(
                        "k", MlUtils.getAbstractType(inputColumns.get(1))));
        unresolvedColumns.add(
                new Schema.UnresolvedPhysicalColumn(
                        "query_vector", MlUtils.getAbstractType(inputColumns.get(2))));
