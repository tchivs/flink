...
/** Utilities for Remote runtime. */
public class RemoteRuntimeUtils {
    ...
    /** Get the response string from a http response. */
    ...
    /** Get the response bytes from a http response. */
    ...
    /** Make a http response from a string. */
    ...
    /** Make a http response from bytes with headers. */
    ...
    public static String getTritonDataType(LogicalType type) {
        ...
        while (typeRoot == LogicalTypeRoot.ARRAY) {
            // Keep un-nesting arrays until we find the inner type.
            type = type.getChildren().get(0);
    ...
    public static Boolean isShapeCompatible(LogicalType logicalType, List<Integer> shape) {
        if (logicalType.getTypeRoot() == LogicalTypeRoot.ARRAY) {
            ...
            return shape.isEmpty() || (shape.size() == 1 && shape.get(0) == 1);
        }
    }
