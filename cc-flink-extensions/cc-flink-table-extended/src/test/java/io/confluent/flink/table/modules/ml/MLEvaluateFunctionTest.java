...
public class MLEvaluateFunctionTest {
    private static final ServiceTasks INSTANCE = ServiceTasks.INSTANCE;

    private TableEnvironment tableEnv;

    private ResolvedCatalogModel testClassificationModel;

    private ResolvedCatalogModel testRegressionModel;
    ...
    void testMLEvaluateClassificationModel() throws Exception {
        ...
        final String functionName = "ml_catalog.db.classification_model_ML_EVALUATE";
        tableEnv.createTemporaryFunction(
                functionName,
                new MLEvaluateFunction(functionName, testClassificationModel.toProperties()));
        final TableResult mlQueryResult1 =
                tableEnv.executeSql(
                        "SELECT ml_catalog.db.classification_model_ML_EVALUATE('ml_catalog.db.classification_model', f1, f2, categorical_label) AS metrics FROM ml_catalog.db.my_table\n;");
    ...
    @Test
    void testMLEvaluateRegressionModel() throws Exception {
        // We need to register the temp function first
        final String functionName = "ml_catalog.db.regression_model_ML_EVALUATE";
        tableEnv.createTemporaryFunction(
                functionName,
                new MLEvaluateFunction(functionName, testRegressionModel.toProperties()));
        final TableResult mlQueryResult1 =
                tableEnv.executeSql(
                        "SELECT ml_catalog.db.regression_model_ML_EVALUATE('ml_catalog.db.regression_model', f1, f2, numerical_label) AS metrics FROM ml_catalog.db.my_table\n;");
        assertThat(mlQueryResult1.getResolvedSchema())
                .isEqualTo(
                        ResolvedSchema.of(
                                Column.physical(
