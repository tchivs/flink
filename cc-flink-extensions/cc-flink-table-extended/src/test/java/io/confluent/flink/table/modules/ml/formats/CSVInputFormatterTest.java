...
/** Tests for {@link CSVInputFormatter}. */
public class CSVInputFormatterTest {
    @Test
    void testGetRequest() throws Exception {
        Schema inputSchema = Schema.newBuilder().column("input", "STRING").build();
        CSVInputFormatter formatter = new CSVInputFormatter(inputSchema.getColumns());
        Object[] args = new Object[] {"input-text-prompt"};
        assertThat(new String(formatter.format(args))).isEqualTo("input-text-prompt");
    }

    @Test
    void testGetRequestWithNull() throws Exception {
        Schema inputSchema =
                Schema.newBuilder()
                        .column("input", "STRING")
                        .column("input2", "FLOAT")
        ...
        assertThat(new String(formatter.format(args))).isEqualTo("input-text-prompt,,2");
    }
