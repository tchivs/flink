 ...
 * transactions.
 */
class IncreasingIdsTransactionAborter extends TransactionAborter {

    private static final Logger LOG =
            LoggerFactory.getLogger(IncreasingIdsTransactionAborter.class);

    private final List<String> prefixesToAbort;
    private final long startingCheckpointIdToAbort;
    private final JavaReflectionProducerFactory<?, ?> producerFactory;

    private @Nullable FlinkKafkaInternalProducer<?, ?> producer = null;

    public IncreasingIdsTransactionAborter(
            String transactionalIdPrefix,
            int subtaskId,
            int parallelism,
            @Nullable KafkaWriterState recoveredState,
            JavaReflectionProducerFactory<?, ?> producerFactory,
            long startingCheckpointIdToAbort) {
        super(transactionalIdPrefix, subtaskId, parallelism, recoveredState, producerFactory);
        this.prefixesToAbort = resolvePrefixesToAbort(transactionalIdPrefix, recoveredState);
        this.startingCheckpointIdToAbort = startingCheckpointIdToAbort;
    ...
    protected void abortLingeringTransactions() {
        for (String prefix : prefixesToAbort) {
            abortTransactionsWithPrefix(prefix, startingCheckpointIdToAbort);
        }
    }
    ...
    private void abortTransactionsWithPrefix(String prefix, long startCheckpointId) {
        for (int subtaskToAbort = this.subtaskIndex; ; subtaskToAbort += numSubtasks) {
            if (abortTransactionOfSubtask(prefix, startCheckpointId, subtaskToAbort) == 0) {
    ...
    private static List<String> resolvePrefixesToAbort(
            String currentTransactionalIdPrefix, @Nullable KafkaWriterState recoveredState) {

        final List<String> prefixesToAbort = new ArrayList<>();
        prefixesToAbort.add(currentTransactionalIdPrefix);

        if (recoveredState != null) {
            final String lastTransactionalIdPrefix = recoveredState.getTransactionalIdPrefix();
            if (!lastTransactionalIdPrefix.equals(currentTransactionalIdPrefix)) {
                prefixesToAbort.add(lastTransactionalIdPrefix);
                LOG.warn(
                        "Transactional id prefix from previous execution {} has changed to {}.",
                        lastTransactionalIdPrefix,
                        currentTransactionalIdPrefix);
            }
        }
