...
public class SqlCreateModel extends SqlCreate implements ExtendedSqlNode {
    ...
    public @Nonnull List<SqlNode> getOperandList() {
        return ImmutableNullableList.of(
                modelName, comment, inputColumnList, outputColumnList, modelOptionList);
    }
    ...
    public void unparse(SqlWriter writer, int leftPrec, int rightPrec) {
        ...
        if (!this.modelOptionList.isEmpty()) {
            writer.keyword("WITH");
            SqlWriter.Frame withFrame = writer.startList("(", ")");
            for (SqlNode modelProperty : modelOptionList) {
                SqlUnparseUtils.printIndent(writer);
