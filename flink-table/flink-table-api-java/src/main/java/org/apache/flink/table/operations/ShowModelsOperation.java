...
/**
 * Operation to describe a SHOW MODELS [FROM [catalog.] database] [[NOT] LIKE pattern] statement.
 */
public class ShowModelsOperation implements ShowOperation {
    ...
    public ShowModelsOperation(String likePattern, boolean useLike, boolean notLike) {
    ...
    public String asSummaryString() {
        StringBuilder builder = new StringBuilder().append("SHOW MODELS");
    ...
    public TableResultInternal execute(Context ctx) {
        ...
        if (preposition == null) {
            models = ctx.getCatalogManager().listModels();
        } else {
            Catalog catalog = ctx.getCatalogManager().getCatalogOrThrowException(catalogName);
            if (catalog.databaseExists(databaseName)) {
                models = ctx.getCatalogManager().listModels(catalogName, databaseName);
            } else {
        ...
        if (useLike) {
            ...
            rows = models.stream().sorted().toArray(String[]::new);
        }
        return buildStringArrayResult("model name", rows);
    }
}
