...
public class JobIDLoggingITCase {
    ...
    void testJobIDLogging(
            ...
            throws Exception {
        JobID jobID = runJob(clusterClient, tmpDir, miniCluster);
    ...
    private static JobID runJob(
            ClusterClient<?> clusterClient, Path tmpDir, MiniCluster miniCluster) throws Exception {
        Configuration configuration = new Configuration();
        Path cpDir = tmpDir.resolve("checkpoints");
        configuration.set(CHECKPOINTS_DIRECTORY, "file://" + cpDir);
        ...
        // wait for 2 checkpoints to check the log message about checkpoint completion notification
        waitForCheckpoint(jobId, miniCluster, 2);
        return jobId;
    }
}
